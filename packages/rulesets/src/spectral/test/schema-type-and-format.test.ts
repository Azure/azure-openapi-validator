...
test("az-schema-type-and-format should find errors", () => {
  ...
  return linter.run(oasDoc).then((results) => {
    ...
    expect(results[0].path.join(".")).toBe("paths./test1.post.parameters.0.schema.allOf.0.properties.prop5.format")
    expect(results[0].message).toBe("Schema with type: string has unrecognized format: email")
    expect(results[1].path.join(".")).toBe("paths./test1.post.parameters.0.schema.properties.prop1.format")
    expect(results[1].message).toBe("Schema with type: integer has unrecognized format: int52")
    expect(results[2].path.join(".")).toBe("paths./test1.post.parameters.0.schema.properties.prop2.properties.prop3.format")
    expect(results[2].message).toBe("Schema with type: string has unrecognized format: special")
    expect(results[3].path.join(".")).toBe("paths./test1.post.parameters.0.schema.properties.prop3.format")
    expect(results[3].message).toBe("Schema with type: boolean should not specify format")
    expect(results[4].path.join(".")).toBe("paths./test1.post.parameters.0.schema.properties.prop4")
    expect(results[4].message).toBe("Schema with type: number should specify format")
