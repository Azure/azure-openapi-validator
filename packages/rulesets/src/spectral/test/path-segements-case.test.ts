...
beforeAll(async () => {
  linter = await linterForRule('PathSegementSubscriptionsAndResoureGroups');
  return linter;
});

test('PathSegementSubscriptionsAndResoureGroups should find errors', () => {
  const oasDoc = {
    swagger: '2.0',
    paths: {
...
test('PathSegementSubscriptionsAndResoureGroups should find no errors', () => {
  const oasDoc = {
    swagger: '2.0',
    paths: {
