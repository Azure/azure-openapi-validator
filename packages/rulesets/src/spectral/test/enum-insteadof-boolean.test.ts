...
test("EnumInsteadOfBoolean should find errors", () => {
      ...
      "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.AzureActiveDirectory/b2cDirectories/{resourceName}":
        {
          ...
          delete: {
            ...
            responses: {
              "200": {
                description:
                  "Response of the URL in Location header if the asynchronous operation is successful. Resource will no longer exist.",
              },
              "202": {
                description:
                  "Resource deletion request accepted. The Location header indicates the URL/location in which to poll for a result of the asynchronous operation. ",
                headers: {
                  Location: {
    ...
    definitions: {
      ...
      B2CTenantResourceProperties: {
        ...
        properties: {
          ...
          goLocal: {
            type: "boolean",
            description: "Is GoLocal add-on enabled for the Azure AD B2C tenant.",
          },
        },
      },
      B2CResourceSKU: {
        ...
        properties: {
          name: {
            ...
            enum: ["Standard", "PremiumP1", "PremiumP2"],
            "x-ms-enum": {
      ...
      B2CTenantResource: {
        ...
        properties: {
          type: {
            ...
            readOnly: true,
            enum: ["Microsoft.AzureActiveDirectory/b2cDirectories"],
            "x-ms-enum": {
              name: "TypeValue",
              modelAsString: true,
              values: [
  ...
  return linter.run(myOpenApiDocument).then((results) => {
    const desiredResults = results.filter((result) => result.message.includes("Booleans properties are not descriptive"))
    expect(desiredResults.length).toBe(4)
    expect(desiredResults[0].path.join(".")).toBe("definitions.OperationDetail.properties.isDataAction")
    expect(desiredResults[1].path.join(".")).toBe("definitions.B2CTenantResourceProperties.properties.goLocal")
    expect(desiredResults[2].path.join(".")).toBe("definitions.NameAvailabilityResponse.properties.nameAvailable")
    expect(desiredResults[3].path.join(".")).toBe("definitions.CreateTenantProperties.properties.goLocal")
  })
})
