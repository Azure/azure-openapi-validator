...
/**
 * this class only handle swagger without external refs, as the inventory's input is a external-refs-resolved swagger
 */
export class ArmUtils {
  ...
  public getResourceByName(modelName: string) {
    ...
    /*if (!this.innerDoc?.definitions || this.innerDoc.definitions[modelName]) {
      const doc = this.inventory.getDocument(this.specPath)
      doc.getReferences()
    }*/
