...
gulp.task('generate/typescript', ['clean/typescript'], function() {
    console.log('Generating the rules imports...');
    const rulesDir = path.join(__dirname, 'src', 'typescript', 'azure-openapi-validator', 'rules');
    const imports = fs.readdirSync(rulesDir).map( file => path.parse(file) ).filter( file => file.ext === '.ts' ).map( file => `import "./${file.name}"` );
    const contents = `
/*---------------------------------------------------------------------------------------------
 *  Copyright (c) Microsoft Corporation. All rights reserved.
 *  Licensed under the MIT License. See License.txt in the project root for license information.
...
${imports.join("\n")}`
    const outputFile = path.join(rulesDir, 'index.ts');
    fs.writeFileSync(outputFile, contents);
});

gulp.task('build/typescript', ['clean/typescript', 'generate/typescript'], function () {
