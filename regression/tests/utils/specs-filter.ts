...
export const allSpecs = specPaths.filter((p: string) => p.includes("resource-manager") && selectedRps.some((rp:string) => p.includes(`specification/${rp}`))).map(f => f.substring(process.cwd().length + 1))
